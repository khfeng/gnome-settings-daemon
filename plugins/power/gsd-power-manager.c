...
#define CURRENT_STATE_FORMAT "(ua((ssss)a(siiddada{sv})a{sv})a(iiduba(ssss)a{sv})a{sv})"
                                    ...
                                    GError **error)
{
        ...
        retval = g_dbus_proxy_call_sync (manager->priv->mutter_displayconfig_proxy,
                                         ...
                                         g_variant_new ("(ssv)",
                                                        ...
                                                        "PowerSaveMode",
                                                        g_variant_new_int32 (enable ? 0 : 3)),
                                         G_DBUS_CALL_FLAGS_NONE,
                                         -1,
...
external_monitor_is_connected (GsdPowerManager *manager)
{
        ...
        while (g_variant_iter_next (monitors_iter, "@((ssss)a(siiddada{sv})a{sv})", &variant))
        {
            g_variant_get (variant, "((ssss)a(siiddada{sv})a{sv})", NULL, NULL, NULL, NULL, NULL, &properties_iter);
...
/*
static void
mutter_displayconfig_proxy_signal_cb (GDBusProxy  *proxy,
                                      const gchar *sender_name,
                                      const gchar *signal_name,
                                      GVariant *parameters,
                                      gpointer user_data)
                                     ...
                                     gpointer user_data)
{
        ...
        /*
        g_signal_connect (manager->priv->mutter_displayconfig_proxy, "g-signal",
                          G_CALLBACK (mutter_displayconfig_proxy_signal_cb),
                          manager);
        */
}
